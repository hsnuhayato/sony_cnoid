...
RTC::ReturnCode_t sony::onExecute(RTC::UniqueId ec_id)
{
  ...
  if(playflag){
    ...
    if(stopflag){//waiting
      ...
      //walkingMotion(m_robot, FT, cm_ref, absZMP, p_Init, p_ref_toe, R_ref_toe, rfzmp, PC, zmpP, count);//toe mode
    }    
    calcWholeIVK(); //command out when test mode
    zmpHandler();
...
inline void sony::prmGenerator(bool &flag)
{
  ...
  else if(flag==1){//keep walking change leg
    prm2Planzmp(FT, p_ref, R_ref, RLEG_ref_p, LLEG_ref_p, LEG_ref_R, rfzmp, zmpP, "A");
    flag=0;
  }
}
